...
pre=json.loads((OUT/"tahap11-r1-r8-prebuild-evidence.json").read_text())
asset_pre=json.loads((OUT/"tahap11-asset-prebuild-evidence.json").read_text())
asset_final=json.loads((OUT/"tahap11-asset-final-evidence.json").read_text())
mut=json.loads((OUT/"tahap11-mutation-evidence.json").read_text())
candidate=json.loads((OUT/"tahap11-candidate-manifest.json").read_text())
patch=json.loads((OUT/"tahap11-patch-request.json").read_text())
plan=json.loads((APP/"ASSURANCE_PLAN_R1_R9.json").read_text())

assert pre["status"]=="PASS" and pre["stage"]=="Tahap 11"
...
assert candidate["status"]=="PUBLIC_TAHAP_11_CANDIDATE"
...
assert patch["status"]=="PUBLIC_TAHAP_11_PATCH_REQUEST"
assert patch["firebaseUsed"] is False
assert patch["manifest"]["parentSignedApkSha256"]==candidate["parentSignedApkSha256"]
assert patch["manifest"]["targetCandidateSha256"]==candidate["candidateSha256"]
assert patch["manifest"]["rollbackBaselineApkSha256"]==candidate["rollbackBaselineApkSha256"]
...
assert hashlib.sha256(payload["canonical"].encode()).hexdigest()==payload["sha256"]
assert payload["sha256"]==patch["manifest"]["payloadSha256"]
assert hashlib.sha256(patch["manifestCanonical"].encode()).hexdigest()==patch["manifestSha256"]
expected_signed=(
    "TBX_REMOTE_PATCH_V1\n"
    +patch["manifestSha256"]+"\n"
    +patch["manifest"]["targetCandidateSha256"]+"\n"
    +patch["manifest"]["rollbackBaselineApkSha256"]+"\n"
)
assert expected_signed==patch["remoteSignedMessage"]
assert patch["requiredSignerCertificateSha256"]=="290fb37d527935766e327781833493400dd647cfc8bdbe433254a2df52e4b8e4"
...
evidence={
    ...
    "evidence":{
        ...
        "developmentParentTahap10SignedIdentity":"PASS",
        "readyAndIrRegression":"PASS",
        "candidateIdentity":"PASS",
        "appPatchRequest":"PASS",
        "remoteVerificationBoundary":"PASS",
        "remoteCertificateTrustAnchor":"PASS",
        "safeRestore":"PASS",
...
(OUT/"tahap11-r1-r9-evidence.json").write_text(
    json.dumps(evidence,indent=2,sort_keys=True)+"\n"
)
print("TAHAP_11_R1_R9 = PASS")
print("APP_PATCH_REQUEST = PASS")
print("REMOTE_VERIFICATION_BOUNDARY = PASS")
print("SAFE_RESTORE = PASS")
print("UNKNOWN = 0")
print("SKIPPED = 0")
