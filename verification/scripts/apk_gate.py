...
ROOT=Path(__file__).resolve().parents[2];SCOPE=json.loads((ROOT/'verification/application_scope.json').read_text(encoding='utf-8'));EVIDENCE_DIR=ROOT/'verification/evidence';ANDROID='{http://schemas.android.com/apk/res/android}'
@dataclass
...
def main()->int:
    ...
    try:
        with zipfile.ZipFile(apk,'r') as z:
            for info in z.infolist():
                name=info.filename;names.append(name);total_uncompressed+=info.file_size;total_compressed+=info.compress_size;normalized=posixpath.normpath(name.replace('\\','/'))
    ...
    except Exception as exc:check('apk-zip-readable',False,str(exc));return finish(a.variant,apk,checks,{})
    duplicates=sorted({n for n in names if names.count(n)>1});check('apk-no-duplicate-entry',not duplicates,f'duplicates={duplicates}');check('apk-canonical-paths',not unsafe,f'unsafe={unsafe}');check('apk-normalized-path-unique',not collisions,f'collisions={collisions}');check('apk-no-pathological-compression',not pathological,f'entries={pathological}');check('apk-required-container-entries','AndroidManifest.xml' in names and 'classes.dex' in names and 'resources.arsc' in names,'manifest/classes/resources');r7_empty=SCOPE['domainScope']['R7']['status']=='CLOSED_EMPTY';check('r7-native-archive-closure',(not r7_empty) or not native,f'native={native}');max_apk=int(SCOPE['budgets'].get('apkBytes',64*1024*1024));max_expanded=int(SCOPE['budgets'].get('apkUncompressedBytes',128*1024*1024));check('apk-size-budget',apk.stat().st_size<=max_apk,f'actual={apk.stat().st_size}, max={max_apk}');check('apk-expanded-size-budget',total_uncompressed<=max_expanded,f'actual={total_uncompressed}, max={max_expanded}')
    manifest_details={}
    try:
        root=ET.parse(manifest_xml).getroot();expected_package=SCOPE['debugApplicationId'] if a.variant=='debug' else SCOPE['applicationId'];actual_package=root.attrib.get('package','');check('final-package-id',actual_package==expected_package,f'actual={actual_package}, expected={expected_package}');version_code=root.attrib.get(ANDROID+'versionCode');version_name=root.attrib.get(ANDROID+'versionName');expected_name='0.1.0-debug' if a.variant=='debug' else '0.1.0';check('final-version-code',version_code=='1',f'actual={version_code}, expected=1');check('final-version-name',version_name==expected_name,f'actual={version_name}, expected={expected_name}');uses_sdk=root.find('uses-sdk');min_sdk=uses_sdk.attrib.get(ANDROID+'minSdkVersion') if uses_sdk is not None else None;target_sdk=uses_sdk.attrib.get(ANDROID+'targetSdkVersion') if uses_sdk is not None else None;req=str(SCOPE['platform']['androidApi']);check('final-min-sdk',min_sdk==req,f'actual={min_sdk}, expected={req}');check('final-target-sdk',target_sdk==req,f'actual={target_sdk}, expected={req}')
        app=root.find('application');check('final-application-present',app is not None,'merged manifest');cleartext=backup=debuggable=theme=app_name=icon=extract_native=None
        if app is not None:
            cleartext=app.attrib.get(ANDROID+'usesCleartextTraffic');backup=app.attrib.get(ANDROID+'allowBackup');debuggable=app.attrib.get(ANDROID+'debuggable','false');theme=app.attrib.get(ANDROID+'theme');app_name=app.attrib.get(ANDROID+'name');icon=app.attrib.get(ANDROID+'icon');extract_native=app.attrib.get(ANDROID+'extractNativeLibs');check('final-cleartext-disabled',cleartext=='false',f'actual={cleartext}');check('final-backup-disabled',backup=='false',f'actual={backup}');check('final-debuggable-contract',(debuggable=='true')==(a.variant=='debug'),f'actual={debuggable}');check('final-framework-theme',theme=='@android:style/Theme.Material.NoActionBar',f'actual={theme}');check('final-application-icon',icon=='@drawable/ic_toolbox',f'actual={icon}');check('final-extract-native-libs-not-forced',extract_native is None,f'actual={extract_native}');check('final-application-class',bool(app_name) and app_name.endswith('.ToolBoxApplication'),f'actual={app_name}')
        permissions=sorted(node.attrib.get(ANDROID+'name','') for node in root.findall('uses-permission'));check('final-permission-universe',not permissions,f'permissions={permissions}');exported=[]
        ...
        exported_ok=len(exported)==1 and exported[0][0]=='activity' and exported[0][1].endswith('.MainActivity');check('final-exported-component-universe',exported_ok,f'exported={exported}');manifest_details={'package':actual_package,'versionCode':version_code,'versionName':version_name,'minSdk':min_sdk,'targetSdk':target_sdk,'permissions':permissions,'exported':exported,'cleartext':cleartext,'allowBackup':backup,'debuggable':debuggable,'theme':theme,'icon':icon,'extractNativeLibs':extract_native,'applicationClass':app_name}
