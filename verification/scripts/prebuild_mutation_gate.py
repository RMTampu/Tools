...
def apply_mutation(root: Path, mutation: Mutation) -> None:
    ...
    if mutation.old not in text: raise RuntimeError(f"mutation anchor not found: {mutation.name}")
    path.write_text(text.replace(mutation.old, mutation.new, 1), encoding="utf-8")

def main() -> int:
    data=archive_head(); results=[]
    for mutation in MUTATIONS:
        with tempfile.TemporaryDirectory(prefix=f"toolbox-mutation-{mutation.name}-") as tmp:
            replica=Path(tmp); extract(data,replica)
            try:
                apply_mutation(replica,mutation)
                completed=subprocess.run([sys.executable,str(replica/mutation.verifier)],cwd=replica,env=os.environ.copy(),stdout=subprocess.PIPE,stderr=subprocess.STDOUT,text=True,check=False,timeout=120)
                results.append(Result(mutation.name,completed.returncode!=0,completed.returncode,completed.stdout[-1600:]))
            except Exception as exc: results.append(Result(mutation.name,False,-1,str(exc)))
    escaped=[r for r in results if not r.detected]; EVIDENCE_DIR.mkdir(parents=True,exist_ok=True)
    payload={"schemaVersion":1,"gate":"PREBUILD_MUTATION_GATE","gitSha":os.environ.get("GITHUB_SHA"),"status":"PASS" if not escaped else "NOT_PROVEN","definedMutations":len(results),"detectedMutations":len(results)-len(escaped),"faultEscape":len(escaped),"results":[asdict(r) for r in results]}
    (EVIDENCE_DIR/"prebuild-mutations.json").write_text(json.dumps(payload,indent=2,sort_keys=True)+"\n",encoding="utf-8")
    if escaped:
        print("PREBUILD_MUTATION_GATE = NOT_PROVEN",file=sys.stderr)
        for item in escaped: print(f"ESCAPE {item.name}: {item.detail}",file=sys.stderr)
        return 1
