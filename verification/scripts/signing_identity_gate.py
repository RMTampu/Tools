...
def normalize(value): return re.sub(r'[^0-9a-f]','',value.lower())
def sha256(path):
    h=hashlib.sha256()
...
def main():
    ...
    contract=json.loads(CONTRACT_PATH.read_text()); policy=contract['release']; ks=a.keystore.resolve(); check('release-keystore-present',ks.is_file() and ks.stat().st_size>0,str(ks)); check('release-alias-present',bool(a.alias.strip()),'alias supplied'); actual=None
    if ks.is_file():
    ...
    failed=[c for c in checks if not c.passed]; EVIDENCE_DIR.mkdir(parents=True,exist_ok=True); payload={'schemaVersion':1,'gate':'SIGNING_IDENTITY_GATE','status':'PASS' if not failed else 'NOT_PROVEN','gitSha':current_git_sha(),'keystoreSha256':sha256(ks) if ks.is_file() else None,'certificateSha256':actual,'checks':[asdict(c) for c in checks],'failed':[c.name for c in failed]}; (EVIDENCE_DIR/'signing-identity.json').write_text(json.dumps(payload,indent=2,sort_keys=True)+'\n')
