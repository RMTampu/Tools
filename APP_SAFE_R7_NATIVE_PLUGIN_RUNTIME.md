...
Untuk target utama `arm64-v8a`, verifikasi instruction set/ABI, library location, ELF machine/class, min Android API/native symbol availability, alignment/calling convention assumptions, STL/runtime linkage, and no accidental dependency on unsupported ABI.
...
Gunakan sanitizer yang kompatibel dengan test environment (ASan/HWASan or equivalent) untuk heap/stack use-after-free, overflow, invalid free, and related memory errors. Android 11 target constraints/tool support must be declared; unsupported sanitizer coverage cannot be silently marked PASS.
...
Track malloc/native heap, threads, mutexes, file/socket handles, callbacks, signal handlers, and lifecycle cleanup. Native resources participate in R2 budget/ownership proof.
...
Library identity/version verification (R6) dilanjutkan dengan tests terhadap actual used API, error behavior, threading, lifecycle, performance/resource assumption, platform API30 behavior, and known breaking-version constraints.
