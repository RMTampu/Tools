...
assert spec["privateImplementationRequired"] is False and spec["privateWiringOnly"] is True
...
assert handoff["integration"]["hostImplementationInPrivateForbidden"] is True and host_spec["productionSourceCount"]==8 and host_contract["privateWiring"]["kernelStateStore"].startswith("thin delegate")
...
stage=sorted((ROOT/"src/main/java/io/toolbox/stagea").glob("*.java")); host=sorted((host_root/"src/main/java/io/toolbox/stagea/android").glob("*.java")); assert len(stage)==6 and len(host)==8
...
evidence={"schemaVersion":2,"projectId":"ToolBox","stageId":"A","componentId":"public.stage-a-foundation","status":"PASS","domainsLoaded":sorted(plan["domains"]),"dependencyProductionSources":6,"stageProductionSources":6,"androidHostProductionSources":8,"totalProductionSources":20,"externalRuntimeDependencies":0,"stageForbiddenAuthorityScan":"PASS","androidHostPrebuildRequired":True,"privateImplementationRequired":False,"privateWiringOnly":True,"publicPrivateReadAccess":0,"firebaseUsed":0,"sourceHashes":hashes}
(BUILD/"prebuild-evidence.json").write_text(json.dumps(evidence,indent=2,sort_keys=True)+"\n"); print("STAGE_A_PREBUILD_ASSURANCE = PASS"); print("PRODUCTION_SOURCE_COUNT=20")
